...
// Error while Depositing again (Second stake call)
...
async function run() {
    ...
    console.log("re_deleg_res => ",re_deleg_res.logs.events);
    const xcf = await staking_contract.query.info();
    console.log("Validator info after re-delegating: ", xcf.info.validators);  
    await delay(1200000);
    await staking_contract.tx.stake(
        { account: other_account, transferAmount: "2000000" }
      );
    await staking_contract.tx.claim_and_stake(
        { account: contract_owner }
    );
    const xwc = await staking_contract.query.info();
    console.log("Validator After Second Deposit: ", xwc.info.validators);
    await mock_validator_contract.tx.update_list(
        {account: contract_owner},
        {list: v1}
    );
    await staking_contract.tx.re_delegate(
        {account: contract_owner}
    )
    const xdcf = await staking_contract.query.info();
    console.log("Validator info after Second re-delegating: ", xdcf.info.validators);
